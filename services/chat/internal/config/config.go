// services/chat/internal/config/config.go
package config
...
func Load(path string) (*Config, error) {
	...
	cfg.Server.Port = 50053 // Using the port from gateway.yaml for chat service
	cfg.Server.Timeout = 30

	// Load configuration file
	if _, err := os.Stat(path); err == nil {
		// Create config loader
		loader := pkgconfig.NewLoader("CHAT", filepath.Dir(path))
